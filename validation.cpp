...
void calcError(std::vector<vpm::Particle> ParticlesNBody, std::vector<vpm::Particle> Particles) {
    ...
    for (size_t i = 0; i < Particles.size(); i++) {
        vpm::Particle& ParticleNBody = ParticlesNBody[i];
        vpm::Particle& Particle = Particles[i];

        double xError = ParticleNBody.PositionX - Particle.PositionX;
        double yError = ParticleNBody.PositionY - Particle.PositionY;
        distanceError += sqrt(xError*xError + yError*yError);

        double xVelError = ParticleNBody.VelocityX - Particle.VelocityX;
        double yVelError = ParticleNBody.VelocityY - Particle.VelocityY;
        velError += sqrt(xVelError*xVelError + yVelError*yVelError);
...
int main() {
    ...
    std::vector<vpm::Particle> ParticlesNBody(N);
    std::vector<vpm::Particle> Particles(N);

    std::ofstream outCSV("error.csv");
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        ParticlesNBody[i] = vpm::Particle(x, y, u, v, omega * ParticleVol * MYSTERY_FRACTION);
    }
...
// #endif
    ...
    for (size_t t = 1; t < nt; t++) {
        ...
        auto DerivativesNBody = vpm::CalcDerivative(ParticlesNBody, L, ParticleRad, Viscosity);
        
        for (size_t i = 0; i < N; i++) {
            vpm::Particle& Particle = ParticlesNBody[i];
            auto [dX, dY, dOmega] = DerivativesNBody[i];
            Particle.PositionX += dX * dt;
            Particle.PositionY += dY * dt;
            Particle.VelocityX = dX;
            Particle.VelocityY = dY;
